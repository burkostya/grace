...
**PURPOSE:** Создание учебного приложения Lesson_17 для демонстрации фреймворка промптов (LDD, Agentic UX, Headless Testing).
**SCOPE:** Генерация точек тригонометрической функции, хранение в SQLite, UI на Gradio, CLI управление.
**KEYWORDS:** [DOMAIN: Education; CONCEPT: Trigonometric Function; TECH: Gradio, SQLite, Plotly]

$START_DOCUMENT_PLAN
### План Документа
**SECTION_GOALS:**
- GOAL Реализовать расчет точек тригонометрической функции y = A * sin(B * x + C) + D => [GOAL_CALCULATION]
...
#### Расчет тригонометрической функции
**TYPE:** GOAL
**KEYWORDS:** [CONCEPT: Math; TECH: Python]
$START_CONTRACT
**PURPOSE:** Вычисление значений y для заданного диапазона x по формуле y = A * sin(B * x + C) + D.
**DESCRIPTION:** На основе параметров A, B, C, D, x_min, x_max генерируется набор точек (x, y).
**RATIONALE:** Базовая математическая логика приложения с использованием тригонометрической функции.
**ACCEPTANCE_CRITERIA:** Корректный расчет y = A * sin(B * x + C) + D для всех x в диапазоне.
$END_CONTRACT
...
$START_CONTRACT
**PURPOSE:** Сохранение и чтение параметров A, B, C, D, x_min, x_max.
...
**DESCRIPTION:** User -> Вводит A, B, C, D, x_min, x_max -> Нажимает "Generate Data" -> Параметры в config.json, точки в БД, таблица обновлена.
