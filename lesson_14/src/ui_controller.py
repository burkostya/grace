# FILE:lesson_14/src/ui_controller.py
# VERSION:1.0.0
...
# INPUT:Данные из компонентов Gradio (слайдеры, выпадающий список, кнопки).
...
# LINKS:[USES_API(Logic): trig_logic; USES_API(DB): database_manager]
# END_MODULE_CONTRACT
#
...
#
# START_MODULE_MAP:
# CLASS[10][Контроллер UI для Lesson 14] => UIController
...
import os
from .trig_logic import calculate_trig_points
...
class UIController:
    ...
    def __init__(self, db_path: str = "lesson_14/trig.db", config_path: str = "lesson_14/config.json"):
    ...
    # SIDE_EFFECTS:Обновляет config.json и сохраняет точки в trig.db
    # KEYWORDS:[PATTERN(Action): Handler; CONCEPT(Data): Calculation]
    # COMPLEXITY_SCORE: 6
    # END_CONTRACT
    def handle_generate_data(self, func_type: str, x_start: float, x_end: float, num_points: int):
        """
        Метод выполняет полный цикл генерации данных: сохранение параметров в конфиг,
        расчет точек тригонометрической функции и их сохранение в базу данных.
        Возвращает DataFrame для отображения в UI.
        """
        logger.info(f"[UI][IMP:9][UIController][handle_generate_data][Start] Генерация данных для {func_type}(x), range=[{x_start}, {x_end}], points={num_points} [START]")
    ...
    # SIDE_EFFECTS:Читает данные из trig.db
    ...
    def handle_draw_graph(self):
        ...
        df = pd.DataFrame(points, columns=['x', 'y'])
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=df['x'], y=df['y'], mode='lines+markers', name=f'{df["x"].iloc[0]:.2f}'))
        fig.update_layout(title=f"Trigonometric Function: {df['x'].iloc[0]:.2f} to {df['x'].iloc[-1]:.2f}", 
