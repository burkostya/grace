# FILE:lesson_14/tests/test_cli.py
# VERSION:1.0.0
# START_MODULE_CONTRACT:
# PURPOSE:Дымовые тесты CLI интерфейса Lesson 14.
...
def test_cli_generate():
    ...
    # BUG_FIX_CONTEXT: Добавлен текущий каталог в PYTHONPATH для корректного импорта lesson_14.src
    env = os.environ.copy()
    env["PYTHONPATH"] = os.getcwd() + os.pathsep + env.get("PYTHONPATH", "")
    
    cmd = ["python", "-m", "lesson_14.src", "generate"]
    ...
    assert os.path.exists("lesson_14/trig.db")
...
def test_cli_export():
    ...
    subprocess.run(["python", "-m", "lesson_14.src", "generate"], cwd=".", env=env)
    # END_BLOCK_PREPARE_DATA

    # START_BLOCK_RUN_EXPORT: [Запуск экспорта]
    out_file = "lesson_14/tests/output_test.csv"
    if os.path.exists(out_file):
        os.remove(out_file)
        
    cmd = ["python", "-m", "lesson_14.src", "export-csv", "--out", out_file]
    result = subprocess.run(cmd, capture_output=True, text=True, cwd=".", env=env)
