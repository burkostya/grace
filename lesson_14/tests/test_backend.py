# FILE:lesson_14/tests/test_backend.py
# VERSION:1.0.0
# START_MODULE_CONTRACT:
# PURPOSE:Тестирование бэкенд-логики (Config, DB, Trig) с проверкой LDD логов.
# SCOPE:Unit-тесты для ConfigManager, DatabaseManager и calculate_trig.
# INPUT:Модули src.
...
# FUNC[10][Тест Trig Logic] => test_trig_logic
# END_MODULE_MAP
...
def test_config_manager(tmp_path, caplog):
    ...
    assert config["function_type"] == "sin"
    
    # 2. Save new
    cm.save_config({"function_type": "cos", "x_start": 0.0, "x_end": 3.14, "num_points": 50})
    
    # 3. Reload
    cm2 = ConfigManager(str(config_file))
    config2 = cm2.load_config()
    assert config2["function_type"] == "cos"
    assert config2["num_points"] == 50
...
# START_FUNCTION_test_trig_logic
# START_CONTRACT:
# PURPOSE:Проверка математических расчетов тригонометрических функций.
# COMPLEXITY_SCORE:4
# END_CONTRACT
def test_trig_logic(caplog):
    """
    Тестирует calculate_trig_points: точность формул sin, cos, tan и обработку ошибок.
    """
    # START_BLOCK_TEST_MATH: [Логика теста математики]
    # 1. Normal case: sin(x)
    points = calculate_trig_points("sin", 0.0, 3.14, 10)
    assert len(points) == 10
    assert abs(points[0][0] - 0.0) < 0.01  # x = 0
    assert abs(points[5][1] - 1.0) < 0.02  # sin(π/2) ≈ 1 (увеличенный tolerance)
    
    # 2. Normal case: cos(x)
    points = calculate_trig_points("cos", 0.0, 3.14, 10)
    assert len(points) == 10
    assert abs(points[0][1] - 1.0) < 0.01  # cos(0) = 1
    
    # 3. Error case: unknown function
    empty_points = calculate_trig_points("unknown", 0.0, 1.0, 10)
    assert len(empty_points) == 0
    
    # 4. Error case: x_min > x_max
    empty_points = calculate_trig_points("sin", 10.0, 5.0, 10)
    assert len(empty_points) == 0
