# FILE:lesson_16/src/cli.py
# VERSION:1.0.0
# START_MODULE_CONTRACT:
# PURPOSE:Точка входа CLI для Lesson_16 (Agentic UX).
# SCOPE:Автономное управление генерацией и экспортом данных.
...
# LAST_CHANGE: [v1.0.0 - Первичное создание модуля CLI.]
# END_CHANGE_SUMMARY
#
# START_MODULE_MAP:
...
def main():
    ...
    - generate: Считывает параметры из config.json, рассчитывает параболу и сохраняет в БД.
    - export-csv: Считывает данные из БД и сохраняет в указанный CSV файл.
    """
    # START_BLOCK_PARSE_ARGS: [Настройка парсера]
    parser = argparse.ArgumentParser(description="Lesson_16 CLI (Agentic UX)")
    ...
    if args.command == "generate":
        ...
        config = load_config()
        df = calculate_parabola(config['a'], config['c'], config['x_min'], config['x_max'])
        save_points(df)
