...
*   **Шаг 5: `PROPOSE_IMPLEMENTATION` (Передача в реализацию)**
    ...
    *   **Обязательный ToDo Checklist для Code-агента:** При передаче задачи вы **ОБЯЗАНЫ** включить в список задач следующие пункты:
        ...
        4. `[ ] IMPLEMENT_TESTS: Создать тесты с Anti-Loop Protocol (счетчик попыток) и выводом логов IMP:7-10`
...
Для минимизации разрыва контекста и исключения повторных чтений ТЗ, Оркестратор обязан ставить субагенту Code **цельную задачу на реализацию функционального среза (Feature Slice) вместе с тестами**.
*   **Запрещено:** Разбивать одну фичу на микрозадачи (отдельно код, отдельно тесты).
