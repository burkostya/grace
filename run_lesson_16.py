# FILE:run_lesson_16.py
# VERSION:1.0.0
# START_MODULE_CONTRACT:
# PURPOSE:Точка входа для запуска Gradio UI Lesson_16.
# SCOPE:Инициализация UI и запуск сервера.
# INPUT:Нет.
...
# LAST_CHANGE: [v1.0.0 - Первичное создание точки входа UI.]
# END_CHANGE_SUMMARY
#
# START_MODULE_MAP:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("lesson_16/app_16.log", encoding='utf-8'),
...
# START_FUNCTION_main
# START_CONTRACT:
# PURPOSE:Инициализация и запуск Gradio UI.
# INPUTS: Нет
# OUTPUTS: Нет
# COMPLEXITY_SCORE: 8
# END_CONTRACT
def main():
    ...
    - Левая: Ввод параметров (a, c, x_min, x_max), кнопки "Generate Data" и "Draw Graph", таблица данных.
    ...
    import gradio as gr
    from lesson_16.src.config_manager import load_config
    from lesson_16.src.ui_controller import on_generate_click, on_draw_click
    from lesson_16.src.database_manager import get_points
    ...
    with gr.Blocks(title="Lesson_16: Parabola Generator") as ui:
        gr.Markdown("# Lesson_16: Parabola Generator (y = ax^2 + c)")
        ...
        with gr.Row():
            ...
            with gr.Column():
                ...
                x_min_input = gr.Number(label="x_min", value=config.get('x_min', -10))
                x_max_input = gr.Number(label="x_max", value=config.get('x_max', 10))
